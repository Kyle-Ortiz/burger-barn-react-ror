...
function Home({user}) {
     ...
     return (
          <div style={{
               ...
               {user ? <button onClick={()=> history.push("/menu")} className="border-black bg-blue-600 mt-6 px-3 py-1 rounded-lg text-xl text-white">Start Order</button> : <button onClick={()=> history.push("/signup")} className="border bg-blue-600"> Sign up here or Login to place an order!</button>}
          </div>
     )
}
